...
ApartmentsMode.prototype.initialize = function () {
    ...
    this.apartmentsEmpty = document.querySelector('#apartments-container .apartments-empty');
    ...
    this._infoPanelSize = { width: 320, height: 220, dirty: true };
    this._infoPanelResizeObserver = null;
    this._markerRevealPasses = 0;
...
ApartmentsMode.prototype.getInfoPanelSize = function () {
    const state = this._infoPanelSize || (this._infoPanelSize = { width: 320, height: 220, dirty: true });
    ...
    state.dirty = false;
    return { width: state.width || 320, height: state.height || 220 };
};

ApartmentsMode.prototype.showEmpty = function (visible) {
    if (!this.apartmentsEmpty) return;
    this.apartmentsEmpty.classList.toggle('visible', !!visible);
    this.apartmentsEmpty.setAttribute('aria-hidden', visible ? 'false' : 'true');
};
...
ApartmentsMode.prototype.ensureMainDataLoaded = function () {
    ...
    this.fetchCsvText(this.mainCsvUrl)
        .then((text) => {
            ...
            if (!parsed.length) this.showEmpty(this._active);
...
ApartmentsMode.prototype.renderMarkers = function (items) {
    ...
    this.apartmentsData = ui?.renderMarkers
        ? ui.renderMarkers(this.apartmentsContainer, this.apartmentsEmpty, items)
        : [];
    this._forceDomUpdate = true;
    this._rectDirty = true;
    this.clearSelectionVisuals();
...
ApartmentsMode.prototype.updateDomPositions = function () {
    ...
    const hideSelectedMarker =
        !!(this.infoPanel && this.infoPanel.classList.contains('visible') && this._selectedApartment);
    this._markerRevealPasses = (this._markerRevealPasses | 0) + 1;
    const renderReady = this._markerRevealPasses >= 2;
    ...
    for (let i = 0; i < this.apartmentsData.length; i++) {
        ...
        if (hideSelectedMarker && item === this._selectedApartment) {
            if (item.visible) {
                item.visible = false;
        ...
        camera.worldToScreen(item.worldPos, this._screenPos);
        const onScreen =
            this._active &&
            this._screenPos.z > 0 &&
        ...
        if (!onScreen) {
            if (item.visible) {
                ...
                item.lastX = NaN;
                item.lastY = NaN;
            } else if (item.revealFrames) {
                item.revealFrames = 0;
            }
            continue;
        }

        const x = rectLeft + this._screenPos.x;
        const y = rectTop + this._screenPos.y;

        const needsReveal = !item.visible;
        const hadPrev = Number.isFinite(item.lastX) && Number.isFinite(item.lastY);

        const dx = hadPrev ? Math.abs(x - item.lastX) : Infinity;
        const dy = hadPrev ? Math.abs(y - item.lastY) : Infinity;

        if (dx > threshold || dy > threshold) {
            item.style.transform = `translate3d(${x}px, ${y}px, 0)` + transformSuffix;
            item.lastX = x;
            item.lastY = y;
        }

        if (needsReveal) {
            const stable = dx <= threshold && dy <= threshold;
            if (!hadPrev || !stable) {
                item.revealFrames = 0;
                pendingReveal = true;
                continue;
            }
            item.revealFrames = (item.revealFrames | 0) + 1;
            if (!renderReady || item.revealFrames < 4) {
                pendingReveal = true;
                continue;
            }
            item.visible = true;
    ...
    if (pendingReveal) this._forceDomUpdate = true;
    this.showEmpty(this._active && !this.apartmentsData.length);
    this.updateInfoPanelPosition();
};
