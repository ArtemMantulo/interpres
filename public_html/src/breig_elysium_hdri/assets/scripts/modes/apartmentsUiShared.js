(function () {
    ...
    const renderMarkers = (container, emptyEl, items) => {
        if (!container) return [];
        ...
        const keepEmpty = emptyEl && emptyEl.parentNode === container ? emptyEl : null;
        if (keepEmpty) container.replaceChildren(keepEmpty);
