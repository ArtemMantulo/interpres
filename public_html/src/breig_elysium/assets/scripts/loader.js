...
export async function loadAssets(app, assets, onComplete, onProgress) {
    let totalBytes = 0;
    let loadedBytes = 0;

    const sizes = await Promise.all(assets.map(asset => {
        return fetch(asset.file.url, { method: 'HEAD' }).then(res => {
    ...
    if (totalBytes === 0) {
        ...
        await Promise.all(assets.map((asset) => {
            return fetchWithProgress(asset.file.url, 0, () => {}).then(buffer => {
                ...
                return new Promise(resolve => {
                    ...
                    asset.ready(() => {
                        loadedFiles++;
                        onProgress?.(loadedFiles / assets.length);
                        resolve();
                    });
                });
            });
    ...
    await Promise.all(assets.map((asset, i) => {
        return fetchWithProgress(asset.file.url, sizes[i], (delta) => {
            loadedBytes += delta;
